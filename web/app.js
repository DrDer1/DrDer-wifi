...
// يدعم الاتصال بالخادم المحلي وميزات التحكم بالأجهزة
// ============================================================
...
class App {
    ...
    static async initializeApp() {
        try {
            ...
            // التحقق من حالة الخادم المحلي بعد تأخير
            setTimeout(async () => {
                await NetworkManager.checkServerAvailability();
                if (NetworkManager.serverAvailable) {
                    UI.showToast('الخادم المحلي متصل - التحكم الكامل متاح', 'success', 3000);
                } else {
                    UI.showToast('الخادم المحلي غير متصل - وضع محدود', 'info', 3000);
                }
            }, 2000);
        } catch (error) {
    ...
    static setupEventListeners() {
        ...
        document.addEventListener('click', (event) => {
            ...
            if (menu && overlay && overlay.style.display === 'block') {
                if (!menu.contains(event.target) && event.target !== overlay) {
                    // لا تغلق إذا تم النقر على زر inside device card
    ...
    static async handleRefreshDevices() {
        ...
        // إذا كان الخادم المحلي متاحاً، استخدمه مباشرة
        if (NetworkManager.serverAvailable) {
            UI.showToast('جاري فحص الشبكة عبر الخادم المحلي...', 'info', 2000);
        } else {
            UI.showToast('جاري فحص الشبكة... (قد يكون أبطأ بدون الخادم المحلي)', 'info', 2500);
        }
    ...
    static handleOnlineStatusChange(isOnline) {
        if (isOnline) {
            ...
            NetworkManager.detectNetwork().then(() => {
                ...
                // إعادة التحقق من الخادم المحلي
                NetworkManager.checkServerAvailability();
            });
        } else {
        ...
        document.body.addEventListener('touchmove', (event) => {
            ...
            while (target && target !== document.body) {
                if (target.classList.contains('app-main') ||
                    ...
                    target.classList.contains('action-menu-content')) {
                    return;
                }
